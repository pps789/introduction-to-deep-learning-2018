...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.random(np.shape(x)) < (1-p)) / (1-p)
        out = mask * x
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            pool = np.max(target, axis=(2, 3))
            for n in range(N):
                for c in range(C):
                    out[n][c][i][j] = pool[n][c]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            target = x[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            argpool = np.unravel_index(np.argmax(target, axis=(2, 3)), target.shape)
            for n in range(N):
                for c in range(C):
                    ii, jj = argpool[n][c]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out, cache = batchnorm_forward(
            np.transpose(x, (0, 2, 3, 1)).reshape((N*H*W, C)),
            gamma,
            beta,
            bn_param
            )
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx, dgamma, dbeta = batchnorm_backward(
            np.transpose(dout, (0, 2, 3, 1)).reshape((N*H*W, C)),
            cache
            )
